  ...
  try {
    ...
    for (const record of subs) {
      ...
      if (daysElapsed >= PAYMENT_REMINDER_AFTER_DAYS && !record.paymentFailureReminderSent) {
        ...
        await sendEmail(
          record.studentEmail,
          'MCQ Music Lessons: action needed on your enrollment payment',
          '<p>Hi ' + escapeHtml(record.studentName) + ',</p><p>Your enrollment payment still hasn\'t gone through. Please update your card details as soon as you can. If this isn\'t resolved within the next ' + daysLeft + ' day' + (daysLeft === 1 ? '' : 's') + ', your slot will be automatically released. You can check or update your enrollment from your <a href="https://mcqmusiclessons.com.au/booking.html#manage-subscription">Manage Enrollment</a> page.</p><p>James</p>'
        );
        await sendEmail(
          JAMES_EMAIL,
